    ...
    return (
		<Modal show={props.show} onHide={onHide} size="lg">
			<Modal.Body className="bg-primary">
				...
				{data && (
					<Row>
						<Col xs={12} lg={4} style={{ overflow: "scroll", maxHeight: "50vh" }}>
						...
						<Col xs={12} lg={8} style={{ overflow: "hidden" }}>
							<YMaps>
								<Map
									state={{
										center: [
											data.reduce((sum, { location }) => sum + location.latitude, 0) / data.length,
											data.reduce((sum, { location }) => sum + location.longitude, 0) / data.length,
										],
										zoom: 11,
										width: 320,
									}}
				...
				<Row className="justify-content-end">
					<Col xs="auto">
						<ButtonComponent disabled={!checked} onClick={handler} isLoading={isLoading} variant="secondary">
							OK
						</ButtonComponent>
					</Col>
				</Row>
