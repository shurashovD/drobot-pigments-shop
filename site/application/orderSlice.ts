...
const getEmpty = (state: IState) => {
    ...
    if ( !state.sdek && !state.address && !state.pvz && !state.tariff ) {
        res.push('2')
    }
...
export const { setActive, setCity, setNameMail, setPhone, setSdek, setSdekTariff, resetOrderPage } = orderSlice.actions
